...
public class TCPServer {
	...
	public void severStart() {
		// 1. 서버의 포트번호 정함
		int port = 8500; // port 번호는 0~65535 사이 지정가능
		...
		InputStream is = null; // 클라이언트 -> 서버 입력용 스티림 변수 
		BufferedReader br = null; // 입력용 보조 스트림 변수
		
		OutputStream os = null; // 서버 -> 클라이언트 출력 스트림 변수  
		...
		try {
			...
			serverSocket = new ServerSocket(port);  // 서보용 소켓을 생성하여 포트와 결합 
			...
			// InputStreamReader: 바이트기반 문자기반 스트림 연결에 사용하는 스트림
			...
			// ==> 스트림을 닫지 않은 상태에서 내용ㅇ르 내보내고 싶은 경우가 있음
			
			// 7 - 2. 클라이언트 => 서버에게 입력(메세지 전송 받기)
			
			String message = br.readLine(); // 클라이언트 메세지르 한 줄 읽어옴.
